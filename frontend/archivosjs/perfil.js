...
async function loadUserData() {
  ...
  if (!user) {
    console.warn('✗ No hay usuario autenticado, redirigiendo a login en 2 segundos...');
  ...
  if (userAvatar) userAvatar.src = user.avatar || 'https://placehold.co/100x100/F0F8FF/0056b3?text=Avatar';
  if (userName) {
    userName.textContent = nombreCompleto;
    console.log('✓ Nombre actualizado:', nombreCompleto);
  }
  if (userEmail) {
    userEmail.textContent = `Email: ${user.correo || user.email || 'correo@example.com'}`;
  ...
  if (userPhone) {
    userPhone.textContent = `Teléfono: ${user.telefono || 'No disponible'}`;
